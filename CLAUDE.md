...
- **`src/config.js`** — the only place that reads `process.env`. Everything else imports `config` from here. Required vars throw at import time if missing (see `required()`); bot-behavior tuning vars (`BOT_RESPONSE_WINDOW_MINUTES`, `BOT_SESSION_GAP_HOURS`, `COURSES_CACHE_TTL_HOURS`, `COURSES_URL`, `INBOX_REFRESH_SECONDS`) have defaults. `parseInboxUsers()` reads `INBOX_USERS` (`nombre:contraseña` pairs, comma-separated) and falls back to the older single-user `INBOX_USER`/`INBOX_PASSWORD` — keep that fallback, an already-deployed instance depends on it to keep booting after a pull.
...
- **`src/whatsapp.js`** — all outbound Graph API calls (`sendTextMessage`, `sendTemplateMessage`), media download (`downloadMedia`), inbound webhook parsing (`parseWebhookEvent`), and HMAC verification (`isValidSignature`).
  - `downloadMedia` is two hops: `GET /{media_id}` returns a temporary URL (5 min), then the bytes are fetched from it. **That URL still needs the bearer token**, which is why the browser cannot load it from an `<img>` tag and `inbox.js` has to proxy it. The `media_id` itself expires after 7 days, so `server.js` downloads on receipt rather than when an employee opens the thread. `parseWebhookEvent` returns a discriminated shape: `{ type: "message", from, text }` for a customer message, `{ type: "echo", to, text }` for a message an employee sent from the official WhatsApp Business app, or `null`. **`isValidSignature` must compare buffer lengths before calling `crypto.timingSafeEqual`** — that function throws on a length mismatch, and it runs outside any try/catch in `server.js`, so skipping the check turns any malformed `x-hub-signature-256` header into a remote process kill. (This was a real bug, fixed; don't reintroduce it.)
- **`src/conversationStore.js`** — every read/write of conversation state, all async and Postgres-backed. Two tables: `conversations` (`wa_id`, `first_message_at`, `last_message_at`, `handed_off`) and `messages` (`wa_id`, `role`, `text`, `created_at`). This is the piece that implements the bot's behavioral rules (below), plus the queries the inbox needs (`listConversations`, `getConversation`, `appendAgentMessage`).
  - `role` is `'user'` | `'model'` | `'agent'`; Gemini only understands user/model, so `'agent'` maps to `'model'` when replaying history. `author` holds which employee sent an `'agent'` message, and is NULL both on bot/customer rows and on agent rows written before multi-user support — the inbox falls back to the generic "Empleado" label, so don't assume it's populated.
  - `listConversations({ search })` matches the phone number or any message text. Two details are load-bearing: `toLikePattern` escapes `%` and `_` (otherwise searching "50%" silently wildcards), and the LATERAL orders matching messages first so a search result previews the message that *matched* rather than the conversation's latest one.
...
- **`src/gemini.js`** — wraps the Gemini SDK. `generateReply(history, userText, coursesContext)` replays `history` (the store's `turns`, mapped to Gemini's `{role, parts}` shape) into a fresh `startChat` call each time rather than keeping a long-lived chat session per user — simpler, and fine since history is already capped. `coursesContext` (from `coursesStore.js`, when present) is appended to only this call's outgoing message, not to what `server.js` later saves via `appendTurn` — so it isn't resent on every subsequent turn.
  - Retries 429/5xx (Gemini returns 503 when the model is momentarily overloaded — this happened in production) twice, with a 1s then 3s backoff; 4xx like a bad key fails immediately, since it never fixes itself. The `ChatSession` is rebuilt per attempt on purpose: it mutates its own history as it sends, so reusing one after a failure risks replaying a half-applied turn.
...
- **`src/inbox.js`** — the employee-facing web inbox mounted at `/inbox`, as an Express Router: conversation list, thread view, and a reply form. Server-rendered HTML strings (the project has no build step and no template engine, deliberately). Two things here are security-critical and easy to break:
  - **Everything rendered goes through `escapeHtml`.** Message bodies are attacker-controlled — a customer can put `<script>` in a WhatsApp message, and it would execute in the employee's browser. This matters more now that the page carries inline `<script>` blocks: an unescaped `</script>` in a message body would break out of them.
  - **`safeEqual` hashes both sides before `timingSafeEqual`**, so mismatched credential lengths neither throw nor leak length through timing. `requireAuth` loops over every configured user without an early `break`, so response time doesn't reveal which names exist, and stashes the matched name on `req.inboxAuthor` for the reply route to record.
  - `autoRefreshScript` reloads the page every `INBOX_REFRESH_SECONDS` (default 30, `0` disables). It is a conditional `setInterval` rather than `<meta http-equiv="refresh">` precisely because it must *skip* reloads: when any `textarea` or `input[type=search]` holds text or has focus (never destroy a half-written reply, never bounce someone off their search results), and when `document.hidden` (a forgotten background tab reloading forever would hold Render's free instance awake and burn the 750 h/month budget). The conversation page also scrolls to the newest message after each load, otherwise every refresh would yank the employee back to the top of the thread.
  - `GET /inbox/media/:messageId` serves stored files. Two things are load-bearing: only `image/*`, `audio/*`, `video/*` and `application/pdf` are served inline — the mime type is **customer-controlled**, so serving an attacker's `text/html` inline on our own origin would be stored XSS with the employee's session; everything else is forced to `attachment` + `application/octet-stream` + `nosniff`. And it uses `res.end(Buffer)`, not `res.send()`: `send()` appends `; charset=utf-8` to binary content types and JSON-serializes anything it doesn't recognize as a Buffer, so a driver returning `bytea` as a `Uint8Array` would deliver `{"0":137,...}` instead of a PNG. (Caught in testing; don't switch it back to `send()`.)
  - `GET/POST /inbox/nuevo` — the cold-outreach flow (`sendTemplateMessage` in `whatsapp.js`). Registered *before* `GET/POST /:waId` in the router so Express matches `/nuevo` literally instead of treating it as a `wa_id`. Order matters: `sendTemplateMessage` is awaited first, and `registerOutboundContact`/`logOutboundContact` only run on success — a rejected template (bad name, not approved) leaves no conversation row and no audit-log entry, so failed attempts don't clutter the inbox as phantom conversations. `registerOutboundContact` inserts with `handed_off = TRUE` and `ON CONFLICT DO NOTHING` — the bot has no context for these campaigns, so any reply must go to a human, and an existing conversation's state is never clobbered. `isOptedOut`/`setOptedOut` (a column on `conversations`) gate only this route — they never block the normal `/reply` route, since opt-out means "don't initiate," not "stop replying to them."

- **`src/embeddedSignup.js`** — **temporary**, mounted at `/es-test`, behind the same Basic Auth as the inbox (it reuses `requireAuth`, which is why that function is exported from `inbox.js`). It exists to answer one question before committing to the multi-week Meta Tech Provider process: does Embedded Signup let us onboard our *own* business portfolio, and does the Coexistence variant of the flow (`featureType: whatsapp_business_app_onboarding`) actually appear? It sends no messages and writes nothing to Postgres — the captured token lives in a module-level variable and dies on restart, deliberately. Inert until `META_APP_ID` and `META_ES_CONFIG_ID` are set. Delete the module and its mount in `server.js` once Coexistence is decided either way.
...
- **The customer is never left in silence.** If Gemini is still failing after its retries, `server.js` sends `FALLBACK_REPLY` instead of nothing. This matters more than it looks: the number no longer has a WhatsApp app on it, so a dropped reply means the customer gets no signal at all and nobody sees the message except in `/inbox`. The fallback deliberately avoids promising that a human will answer *instead of* the bot — outages are brief and the bot should still handle the next message.
- **Outbound sends happen before the DB write** (`sendTextMessage` then `appendBotMessage`, matching `/inbox`'s reply route). A stored-but-undelivered message would show in the inbox as if the customer had been answered, and the employee would never follow up; delivered-but-unlogged is the recoverable direction.
- **Media messages never reach Gemini.** `server.js` returns early for a media message with no caption: the model cannot see the file, so a reply would be invented. With a caption, the caption is treated as the message and the bot answers normally. The row is stored either way — including when the download fails or exceeds `MEDIA_MAX_MB`, since the employee still needs to know something arrived.
...
- **The Graph API version expires on a clock.** Meta retires each version roughly two years after release, and calls to a retired one fail outright — this is a scheduled outage, not a deprecation warning. It's `config.graphApiVersion` (env `GRAPH_API_VERSION`), currently `v23.0`, which lapses **2027-10-08**. The pin was `v20.0` hardcoded in `whatsapp.js` until it was three weeks from expiring, so check [the version table](https://developers.facebook.com/docs/graph-api/changelog/versions/) rather than assuming the current value is safe.
