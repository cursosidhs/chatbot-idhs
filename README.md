...
1. **Solo responde al arrancar la conversación**: el bot contesta durante los primeros `BOT_RESPONSE_WINDOW_MINUTES` (**300 = 5 horas** por defecto) desde el primer mensaje del cliente en una conversación. Pasado ese tiempo deja de responder, pero **no se queda mudo**: manda una vez el aviso de que alguien del equipo se va a contactar de 10 a 17 hs (constante `AFTER_HOURS_REPLY` en `server.js`, ahí se edita el horario). Ese aviso sale **una sola vez por sesión** — cinco mensajes fuera de la ventana no generan cinco avisos — y el contador se reinicia cuando arranca una conversación nueva. Si un empleado ya tomó la conversación, no sale ningún mensaje automático.
